...
const BookingConfirmationPage = () => {
    ...
    const handlePaymentAndBook = async () => {
        ...
        try {
            ...
            if (response.data.isSuccess) {
                setSuccess('Đặt sân thành công! Bạn sẽ được chuyển về trang quản lý.');
                setTimeout(() => {
                    navigate('/staff/manage-bookings');
                }, 3000);
            } else {
    ...
    if (!bookingDetails) {
        return (
            <Container>
                <Alert severity="error">Không có thông tin đặt sân. Vui lòng chọn lại.</Alert>
                <Button onClick={() => navigate('/staff')}>Về trang Dashboard</Button>
            </Container>
        );
    }
